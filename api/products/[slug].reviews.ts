...
const ReviewSchema = z.object({ rating: z.number().min(1).max(5), title: z.string().optional(), body: z.string().optional() })
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    const reviews = await prisma.review.findMany({ where: { product: { slug } }, include: { user: true }, orderBy: { createdAt: 'desc' } })
  ...
  if (req.method === 'POST') {
    ...
    const data: any = { rating: parse.data.rating, title: parse.data.title, body: parse.data.body, productId: product.id }
